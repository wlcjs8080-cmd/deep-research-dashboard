...
- (2026-03-02) GPT-5 이상 모델은 temperature 파라미터를 지원하지 않음. API 호출 시 temperature 삭제할 것.
- (2026-03-02) GPT-5 이상 모델은 max_tokens 대신 max_completion_tokens를 사용해야 함.
...
- (2026-03-02) 상세근거 텍스트 가독성 필터 추가됨:
  - 볼드(**텍스트**) 제거: .replace("**", "")
  - 마크다운 헤더 제거: re.sub(r'^#{1,3}\s*', '', text, flags=re.MULTILINE)
  - 마침표 기준 문장 단위 줄바꿈: re.sub(r'(?<=[.다])\s+', '\n', text) — LLM이 한 덩어리로 반환할 때 \n→\n\n 변환이 무효하므로 마침표 기준으로 분리
...
- (2026-03-02) GPT-5 이상 모델에서 temperature 파라미터를 넣으면 API 에러 발생. 반드시 삭제해야 함.

- (2026-03-02) GPT-5 이상 모델에서 max_tokens → max_completion_tokens로 파라미터명이 변경됨.

- (2026-03-02) LLM이 상세근거를 한 덩어리 텍스트로 반환하면 \n→\n\n 변환이 무효함. 마침표 기준 re.sub 분리가 필요.

- (2026-03-02) Anthropic API 응답은 JSON을 마크다운 코드블록(```json ... ```)으로 감쌀 수 있음. re.search(r"\{.*\}", raw, re.DOTALL)로 추출 필수. (기존 교훈과 중복이지만 강조 목적으로 재기록)
